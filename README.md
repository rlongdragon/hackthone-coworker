...
```bash
git clone git@github.com:rlongdragon/coworkers.git && cd coworkers
docker compose -f docker-compose.postgres.yml up -d   # 應用 DB(pgvector),host :5433
docker compose -f docker-compose.langfuse.yml up -d   # LLM 觀測,UI http://localhost:3001
...
Langfuse 會自動完成初始化(帳號與 API key 都寫在 compose 檔內,登入帳密見檔案註解)。

### 2. 設定並啟動應用

```bash
cd web
cp .env.example .env.local   # 填 LLM_* 三個值;LANGFUSE pk/sk 抄 compose 檔的 LANGFUSE_INIT_PROJECT_*
...
> ⚠️ `docker-compose.langfuse.yml` 內建的是**本機開發用**密鑰(SALT / encryption key / API keys / admin 密碼)。正式部署前務必全部重新產生。
